...
df.melt$error = rnorm(n = nrow(df.melt),mean = 0.00, sd = 0.01)
df.melt$ve = with(df.melt, value+error)
df.melt$ve[df.melt$ve<0]=0.0001
df.melt$ve[df.melt$ve>1]=.9999

saveRDS(df.long,'set3a.RDA')
saveRDS(df.melt,'set4a.RDA')
...
#ggplot(dataP, aes(x = tAge,y = ve,group=RID,col=factor(variable)))+geom_line()+facet_grid(Syndrome~variable)+xlim(50,90)+
#  scale_color_manual(values = myColors[2:4])

#ggplot(dataP, aes(x = oTime,y = ve,group=RID,col=factor(variable)))+geom_line()+facet_grid(Syndrome~variable)+xlim(0,7)+
 # scale_color_manual(values = myColors[2:4])
...
l23 = ggplot(data= dataP,aes(x = oTime,y=ve,group=RID,color=Syndrome))+geom_line()+geom_point()+
  ...
  scale_color_manual(values = myColors[6:7])+xlim(0,7)+
  theme(legend.position='bottom',plot.title = element_blank(),legend.title=element_blank(),
        axis.title.x = element_text(size = 15))
